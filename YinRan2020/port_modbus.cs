    ...
    public class port_moudbus
    {
        ...
        private void sp_DataReceived(object sender,SerialDataReceivedEventArgs e)
        {
            ...
            if (array[1] == 6)
            {
                ...
                Device_Data.chejian3_com1_DT[array[0], start_address] = array[5];
            ...
            if (send_machine_num == array[0] && send_gongnengma == array[1] && send_is == true && receive_is == false)
            {
                ...
                if (array[1] == 3)
                {
                    ...
                    {
                        Device_Data.chejian3_com1_DT[array[0], start_address + i] = array[3 + i * 2] * 256 + array[3 + i * 2 + 1];
                        int a = Device_Data.chejian3_com1_DT[1, 0];
                ...
                if (array[1] == 1)
                {
                    ...
                    for (int i = 0; i < data_num; i++)
                    {
                        if (i % 2 == 0)
                        {
                            Device_Data.chejian3_com1_jicun[array[0], start_address / 2 * 16 + i / 2] = (int)array[3 + i + 1] * 256 + array[3 + i];
                            //RealTime_data.R[array[0], start_address / 2 * 16 + i / 2] = (int)array[3 + i + 1] * 256 + array[3 + i];//!!!!!!!!!!!!!!!!!!! 43 65 if i%2 ==0

                        }
                        int b = Device_Data.chejian3_com1_jicun[array[0], start_address / 2 * 16 + i / 2];
                        string erjinzhi = Convert.ToString(b, 2).PadLeft(16, '0');

                        for (int j = 0; j < 16; j++)
                        {
                            if (int.Parse(erjinzhi.Substring(15 - j, 1)) == 0)
                            {
                                Device_Data.chejian3_com1_R[array[0], start_address / 2 * 16 + i / 2 * 16 + j] = false;
                            }
                            if (int.Parse(erjinzhi.Substring(15 - j, 1)) != 0)
                            {
        ...
        public void sp_config(string COM_string, string botelv, string jiaoyan, string datalength, string stopbit)
        {
            if (botelv != "" && jiaoyan != "" && datalength != "" && stopbit != "" && sp.IsOpen == false)
            {
                ...
                if (botelv == "2400kps") sp.BaudRate = 2400;
                if (botelv == "4800kps") sp.BaudRate = 4800;
                if (botelv == "9600kps") sp.BaudRate = 9600;
                if (botelv == "19200kps") sp.BaudRate = 19200;
                if (botelv == "38400kps") sp.BaudRate = 38400;
                if (jiaoyan == "odd") sp.Parity = System.IO.Ports.Parity.Odd;
                if (jiaoyan == "even") sp.Parity = System.IO.Ports.Parity.Even;
                if (jiaoyan == "none") sp.Parity = System.IO.Ports.Parity.None;
